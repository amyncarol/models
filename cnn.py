...
MAX_N_SITES = 10         #the maximum of number of sites among all structures
FEATURE_LENGTH = 40  # the number of features for each site(or element), to be determined, can be a hyper parameter
...
def inputs(eval_data):
  ...
  Returns:
    sites: 3D tensor of [batch_size, MAX_N_SITES, FEATURE_LENGTH] size. For sites without atom, fill in zeros
    ground_truth: 1D tensor of [batch_size] size.
    graphs: a python dictionary that stores the connectivity, this needs not to be a tensor since it doesn't change
